...
const chartData = [
  {
    ...
    options: {
      ...
      "series": [{
        "data": [31.9, 23.5, 23, 17.7, 15, 9.9, 24.8],
        "type": "bar",
  ...
  },
  {
    ...
    options: {
      ...
      "series": [{
        "data": [13.4, 17.5, 29.3, 7.4, 7.6, 24.8],
        "type": "bar",
        "label": {"show": true, "formatter": "{c}%"}
      }]
    }
  },
  {
    ...
    options: {
      ...
      "series": [{
        "data": [31.3, 15.4, 10.7, 9.7, 8.2, 5.1, 4.5, 3.9, 3, 5.5, 6],
        "type": "bar",
        "label": {"show": true, "formatter": "{c}%"}
      }]
    }
  ...
  {
    title: '',
    subtitle: '除了最盛行的投資詐騙，橫跨所有世代；其餘以年齡層來看，50歲以下接觸的樣態多集中在購物詐騙、假帳單詐騙；50歲以上則以假冒公務機關和假冒親友詐騙訊息最多。',
    options: {
      tooltip: {
        trigger: 'axis',
      ...
      legend: {
        show: true,
        itemGap: 15,
        data: ['投資詐騙', '購物詐騙', '中獎/優惠詐騙', '假帳單詐騙', '假冒親友詐騙', '假冒公務機關', '勒索恐嚇詐騙', '假融資詐騙', '交友/愛情詐騙'],
      },
      series: [
        {
